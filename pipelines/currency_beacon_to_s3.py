...
def fetch_historical_exchange_rates(start_date:str, end_date:str, base_currencies:list[str], bucket_name:str, path_prefix) -> None:
    """"""
    currency_beacon = CurrencyBeacon()
    s3 = S3()

    historical_rates = currency_beacon.get_timeseries_rates(start_date, end_date, base_currencies)
    upload_file = s3.upload(historical_rates, bucket_name, path_prefix)
    logger.info(f"✅ Successfully uploaded [{','.join(base_currencies)}] data to {upload_file}")
...
def main() -> None:  # noqa: D103
    ...
    historical_parser = subparsers.add_parser(
        "historical", help="Fetch the historical exchange rates."
    )
    historical_parser.add_argument(
        "--start_date",
        required=True,
        help="the start date of the historical extract"

    )
    historical_parser.add_argument(
        "--end_date",
        required=True,
        help="the end date of the historical extract"
    )
    historical_parser.add_argument(
        "--base_currencies",
        required=True,
    ...
    if args.mode == "current":
        ...
        fetch_historical_exchange_rates(args.start_date, args.end_date, args.base_currencies, args.bucket_name, args.path_prefix)
